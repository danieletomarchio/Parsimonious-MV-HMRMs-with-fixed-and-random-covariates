...
Eigen.FMR_HMM_init <- function(Y, X, k, mod.row.Y = "all", mod.col.Y = "all", nstartR = 50, nThreads = 1) {
  r_Pars_init <- function(Y, X, k, mod.row.Y, mod.col.Y, nstartR = 50) {
    ...
    Xresh <- array(X, dim = c(q - 1, r, num)) # data reshape
    Yresh <- array(Y, dim = c(p, r, num)) # data reshape
    ...
    post2 <- array(NA, c(k, k, num0, t - 1)) # transtion probabilities
    f <- array(NA, c(num0, t, k, nstartR))
    A <- B <- array(NA, c(num0, t, k, nstartR)) # forward and backward variables
      ...
      tryCatch(
        {
          ...
          A[, 1, , h] <- matrix(rep(prior[h, ], each = num0), ncol = k) * f[, 1, , h] # log(foo) + lscale
          B[, t, , h] <- 1
          if(k==1){
  ...
  for (g in 1:length(k)) {
    print(paste("Initializing Parsimonious Matrix-Normal HMM-FMRs with k =", k[g]))
...
Eigen.FMR_HMM_fit <- function(Y, X, init.par = NULL, tol = 0.001, maxit = 500, nThreads = 1) {
  ...
  Pars_MVN_FMR <- function(Y, X, k, init.par = NULL, mod.row.Y = NULL, mod.col.Y = NULL, tol = 0.001, tol2 = 0.001, maxit = 500, maxit2 = 100) {
    ...
    while (check < 1) {
      ...
      A[,1,] <- matrix(rep(prior,each=num0),ncol=k)*f[,1,] # log(foo) + lscale
      B[,t,] <- 1
      for(T in 2:t){
    ...
    return(list(
      ...
      PI = round(PI,digits = 3), A = A, B = B, 
      loglik = loglik.new, mark = mark, check = check, npar = npar, iter = m.iter, time = time,
  ...
  for (g in 1:length(k)) {
    ...
    print(paste("Fitting Parsimonious Matrix-Normal HMM-FMRs with k =", k[g]))
